...
/**
 * This Closs implements the Rule "GET must be used to retrieve a representation of a resource" for GET Requests
 */
public class GetResourceRule implements IRestRule{
    ...
    private static final List<RuleSoftwareQualityAttribute> SOFTWARE_QUALITY_ATTRIBUTES = List.of(RuleSoftwareQualityAttribute.MAINTAINABILITY, RuleSoftwareQualityAttribute.COMPATIBILITY,
    ...
    /**
     * checks if a given GET requests contains a request body and return a Violation Object if it contains a request body.
    ...
    private Violation checkForRequestBody(Operation getRequest, String path) {
        if (getRequest.getRequestBody() == null)
            return null;
        //Get requests should not have a request body
        return new Violation(this, locMapper.getLOCOfPath(path), ImprovementSuggestion.GET_RESOURCE_REQUESTBODY, path,ErrorMessage.GET_RESOURCE_REQUESTBODY);
    }
    ...
    private Violation checkForValidResponse(Operation getRequest, String path) {
        ...
        if (okResponse != null){
            if (okResponse.getContent() == null || okResponse.getContent().isEmpty()){
                return new Violation(this, locMapper.getLOCOfPath(path),ImprovementSuggestion.GET_RESOURCE, path, ErrorMessage.GET_RESOURCE);
            }
            ...
            return new Violation(this, locMapper.getLOCOfPath(path),ImprovementSuggestion.GET_RESOURCE_MISSING_RESPONSE, path, ErrorMessage.GET_RESOURCE_MISSING_RESPONSE);
        }
